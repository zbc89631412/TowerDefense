...
public class Enemy : MonoBehaviour
{
    ...
    if(isBeingShooted){
         print("is being shooted "+isBeingShooted);
    }
    ...
    if (gunShotTime >= 0.1f)
        ...
        if(detect_player()||isBeingShooted){
...
//        print(randomAngle);
        if(Physics.Raycast(end.transform.position,
        rotation*(end.transform.position-start.transform.position).normalized,out rayHit,100.0f))
        {
            GameObject bulletHoleObject = Instantiate(bulletHole,
            rayHit.point+rayHit.collider.transform.up*0.01f,rayHit.collider.transform.rotation);
