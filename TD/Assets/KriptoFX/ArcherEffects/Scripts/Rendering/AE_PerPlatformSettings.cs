...
public class AE_PerPlatformSettings : MonoBehaviour
{
    ...
    void LWRP_Rendering_Check (Camera cam)
    {
        ...
        if (cam == null) return;
        var mobileLwrpDistortion = cam.GetComponent<AE_LWRP_RenderDistortion>();
        if (mobileLwrpDistortion == null) mobileLwrpDistortion = cam.gameObject.AddComponent<AE_LWRP_RenderDistortion>();
